...
var override = function override(configs) {
  configs.forEach(function (_ref2) {
    var _ref3 = _toArray(_ref2),
      host = _ref3[0],
      methods = _arrayLikeToArray(_ref3).slice(1);
...
var fixRouterContext = function fixRouterContext() {
  var tryFixFromHost = function tryFixFromHost(host) {
    if (!_isExist(host)) {
      return;
    }
    Object.keys(host).forEach(function (key) {
      ...
      if (_isExist(ctx.Provider) || _isExist(ctx.Consumer)) {
        fixContext(ctx);
      }
    });
  };
  ...
  return _createClass(ProviderBridge, [{
    ...
    value: function render() {
      ...
      var _ctxValues2 = _toArray(ctxValues),
        ...
        restValues = _arrayLikeToArray(_ctxValues2).slice(1);
...
var AliveScope = /*#__PURE__*/function (_Component) {
  function AliveScope() {
    ...
    _defineProperty(_this, "waitForCache", function (id) {
      var retry = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 30;
      return new Promise(function (resolve) {
        ...
        setTimeout(function () {
          _this.waitForCache(id, retry - 1).then(resolve);
        }, 0);
      });
    });
    ...
    _defineProperty(_this, "keep", function (id, params) {
      return _this.update(id, _objectSpread2({
        id: id
      }, params)).then(function () {
        return _this.waitForCache(id);
      });
    });
  ...
  function KeepAlive(props) {
    ...
    _defineProperty(_this, "init", function () {
      ...
      _helpers.keep(id, _objectSpread2({
        ...
        _this.waitForPlaceholder().then(function (isReady) {
          if (!isReady) {
            return;
          }
          _this.inject();
    ...
    _defineProperty(_this, "waitForPlaceholder", function () {
      var retry = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 30;
      return new Promise(function (resolve) {
        console.log('waitForPlaceholder', retry);
        if (_this.placeholder || retry <= 0 || _this.unmounted) {
          resolve(Boolean(_this.placeholder));
          return;
        }
        setTimeout(function () {
          _this.waitForPlaceholder(retry - 1).then(resolve);
        }, 0);
      });
    });
...
exports.fixRouterContext = fixRouterContext;
